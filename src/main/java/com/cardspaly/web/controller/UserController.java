...
import com.cardspaly.web.service.LogService;
import com.cardspaly.web.service.MenuService;
import com.cardspaly.web.service.RoleService;
import com.cardspaly.web.service.TokenIpaddressService;
import com.cardspaly.web.service.UserService;
import com.cardspaly.web.util.StringUtil;
...
public class UserController {
	...
	@Resource
	private TokenIpaddressService tokenIpaddressService;
	...
	public Map<String, Object> login(String imageCode, @Valid User user, BindingResult bindingResult,
		...
		try {
			...
			TokenIpaddress tokenIpaddress = new TokenIpaddress();
			tokenIpaddress.setToken(Currentsession.hashCode());
			tokenIpaddressService.save(tokenIpaddress);
	 ...
	 */
	@ResponseBody
	@PostMapping("/checkfortoken")
	public Map<String, Object> checkForToken(Integer token, String ipaddress) {
		Map<String, Object> map = new HashMap<String, Object>();
		List<TokenIpaddress> list = tokenIpaddressService.find();

		for (TokenIpaddress tokenIpaddress : list) {
			if (token.equals(tokenIpaddress.getToken())) {
				map.put("success", true);
				tokenIpaddressService.updateIpaddressByToken(token, ipaddress);
				return map;
			}
		}
		map.put("success", false);
		map.put("errorInfo", "token不匹配！");
		return map;
	}
